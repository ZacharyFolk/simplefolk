...
,'post__in'  => get_option( 'sticky_posts' )));
	if($query->have_posts() && get_option( 'sticky_posts' )){
			...
			$image_attributes = wp_get_attachment_image_src($attachment_id,'pixgraphy_slider_image');
				...
				$remove_link = $pixgraphy_settings['pixgraphy_slider_link'];
					if($remove_link == 0){
						if ($title_attribute != '') {
							$pixgraphy_page_sliders_display .= '<h2 class="slider-title"><a href="'.esc_url(get_permalink()).'" title="'.the_title('', '', false).'" rel="bookmark">'.get_the_title().'</a></h2><!-- .slider-title -->';
						}
					}else{
  ...
  <nav class="slider-button" role="navigation" aria-label="'.esc_attr__('Slider Menu','pixgraphy').'"> </nav>
  <!-- end .slider-button -->
</div>
<!-- end .main-slider -->';
...
function pixgraphy_scripts() {
	...
	wp_enqueue_script('pixgraphy-skip-link-focus-fix', get_template_directory_uri().'/js/skip-link-focus-fix.js', array('jquery'), false, true);
