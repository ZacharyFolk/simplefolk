...
class WPSDB extends WPSDB_Base
{
	...
	function get_sql_dump_info($migration_type, $info_type)
	{
		...
		return sprintf(
			'%s%s%s-%s-%s-%s.sql',
			$this->get_upload_info($info_type),
			$ds,
	...
	function respond_to_remote_finalize_migration()
	{
		$filtered_post = $this->filter_post_elements($_POST, array(
			'action', 'intent', 'url', 'key', 'form_data', 'prefix',
			'type', 'location', 'tables', 'temp_prefix'
		));
		if (!$this->verify_signature($filtered_post, $this->settings['key'])) {
	...
	function finalize_migration()
	{
		...
		foreach ($temp_tables as $table) {
			...
			$sql .= "\n";
			$sql .= 'RENAME TABLE ' . $this->backquote($table) . ' TO ' . $this->backquote(substr(
				$table,
				strlen($temp_prefix)
			)) . ';';
			$sql .= "\n";
		...
		$preserved_options_data = $wpdb->get_results(sprintf(
		...
		foreach ($preserved_options_data as $option) {
			...
			$sql .= $wpdb->prepare("INSERT INTO `{$_POST['prefix']}options` ( `option_id`, `option_name`, `option_value`,
`autoload` ) VALUES ( NULL , %s, %s, %s );\n", $option['option_name'], $option['option_value'], $option['autoload']);
		}
	...
	function ajax_process_chunk()
	{
		...
		if ($this->settings['allow_push'] != true) {
			$result = $this->end_ajax(__('The connection succeeded but the remote site is configured to reject push connections.
You can change this in the "settings" tab on the remote site. (#133)', 'wp-sync-db'));
			return $result;
		}
	...
	function ajax_migrate_table()
	{
		...
		// Pull and push need to be handled differently for obvious reasons, trigger different code depending on the migration
		// intent (push or pull)
	...
	{
		$filtered_post = $this->filter_post_elements($_POST, array(
			'action', 'intent', 'url', 'key', 'table', 'form_data',
			'stage', 'bottleneck', 'prefix', 'current_row', 'dump_filename', 'last_table', 'gzip', 'primary_keys',
			'path_current_site', 'domain_current_site'
		));
	...
	function respond_to_process_pull_request()
	{
		$filtered_post = $this->filter_post_elements($_POST, array(
			'action', 'intent', 'url', 'key', 'table', 'form_data',
			'stage', 'bottleneck', 'prefix', 'current_row', 'dump_filename', 'pull_limit', 'last_table', 'gzip', 'primary_keys',
			'path_current_site', 'domain_current_site'
		));
		...
		if ($this->settings['allow_pull'] != true) {
			$result = $this->end_ajax(__('The connection succeeded but the remote site is configured to reject pull connections.
You can change this in the "settings" tab on the remote site. (#132)', 'wp-sync-db'));
			return $result;
		}
	...
	// Does a quick check to make sure the verification string is valid and also opens / creates files for writing to (if
	// required)
	function ajax_initiate_migration()
	{
		...
		if ($_POST['intent'] == 'savefile') {
			...
			if (false === $return) {
				$error_msg = __(
					'Failed attempting to unserialize the response from the remote server. Please contact support.',
					'wp-sync-db'
				);
	...
	// End point for the above remote_post call, ensures that the verification string is valid before continuing with the
	//migration
	function respond_to_remote_initiate_migration()
	{
		...
		if ($this->verify_signature($filtered_post, $this->settings['key'])) {
			if (isset($this->settings['allow_' . $_POST['intent']]) && (true === $this->settings['allow_' . $_POST['intent']] ||
				1 === $this->settings['allow_' . $_POST['intent']])) {
				$return['error'] = 0;
			} else {
				$return['error'] = 1;
				if ($_POST['intent'] == 'pull') {
				...
				$return['message'] = sprintf(__('The connection succeeded but the remote site is configured to reject %s connections.
You can change this in the "settings" tab on the remote site. (#110)', 'wp-sync-db'), $intent);
			}
		} else {
			$return['error'] = 1;
	...
	function ajax_verify_connection_to_remote_site()
	{
		...
		if (isset($_POST['convert_post_type_selection']) && '1' == $_POST['convert_post_type_selection']) {
			...
			$this->settings['profiles'][$profile]['select_post_types'] = array_values(array_diff(
				$response['post_types'],
				$this->settings['profiles'][$profile]['select_post_types']
			));
	...
	function respond_to_verify_connection_to_remote_site()
	{
		...
		if (!$this->verify_signature($filtered_post, $this->settings['key'])) {
			$return['error'] = 1;
			$return['message'] = $this->invalid_content_verification_error . ' (#120) <a href="#" class="try-again js-action-link">'
				. __('Try again?', 'wp-sync-db') . '</a>';
		...
		if (!isset($this->settings['allow_' . $_POST['intent']]) || $this->settings['allow_' . $_POST['intent']] != true) {
			...
			$return['message'] = sprintf(__('The connection succeeded but the remote site is configured to reject %s connections.
You can change this in the "settings" tab on the remote site. (#122) <a href="#" class="try-again js-action-link">Try
    again?</a>', 'wp-sync-db'), $intent);
			$result = $this->end_ajax(serialize($return));
			return $result;
		}
